...
class Embedding:
    def __init__(self):
        ...
        self.captioning_model = BlipForConditionalGeneration.from_pretrained("Salesforce/blip-image-captioning-base").to(self.device)
    ...
    def generate_caption(self, images):
        try:
            ...
            for image in images:
                ...
                input = self.captioning_processor(image, return_tensors="pt", max_length=100, truncation=True).to(self.device)
    ...
    def get_train_image_embeddings(self, pair_list):
        ...
        try:
            ...
            for image, caption in pair_list:
                ...
                input_caption = self.clip_processor(text=[caption], return_tensors="pt", padding=True, max_length=100, truncation=True).to(self.device)
                input_image = self.clip_processor(images=image, return_tensors="pt", padding=True, max_length=100, truncation=True).to(self.device)
    ...
    def input_text_embedding(self, text: str):
        try:
            input_text = self.clip_processor(text=[text], return_tensors="pt", padding=True, max_length=100, truncation=True).to(self.device)
